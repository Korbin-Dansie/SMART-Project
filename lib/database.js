...
function addTableData() {
  ...
  runSQL("CALL create_application(6, 1, '2005-08-14', 1.1, 1.1, TRUE, FALSE, 'essay', @id)", "Inserted sample Application");
  ...
  runSQL(`CALL add_feeding(1, 1, '${new Date().toISOString().slice(0, 10).replace('T', ' ')}', 1)`, "Add lunch for student 1");
  runSQL(`CALL add_feeding(2, 1, '${new Date().toISOString().slice(0, 10).replace('T', ' ')}', 1)`, "Add breakfast for student 2");
  runSQL(`CALL add_feeding(2, 2, '${new Date().toISOString().slice(0, 10).replace('T', ' ')}', 1)`, "Add lunch for student 2");
