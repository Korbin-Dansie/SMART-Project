...
function createStoredProcedures() {
  ...
  ")\n" +
  "BEGIN\n" +
  " SELECT start_time, end_time, day_of_week.name, class_time.group\n" +
  " FROM class_time\n" +
  ...
  "BEGIN\n" + 
  "	\n" + 
  "    SELECT student.student_id, person.first_name, person.last_name, class_time.class_time_id\n" + 
  "    FROM student\n" + 
  "    JOIN student_schedule ON student_schedule.student_id = student.student_id\n" + // What kind of join? Keep all students, duplicate when they have multiple class times
  "    JOIN class_time ON student_schedule.class_time_id = class_time.class_time_id\n" + // What kind of join?
  "    JOIN application ON student.student_id = application.student_id\n" + // What kind of join? Only want students
  "    JOIN person ON application.person_id = person.person_id\n" + // What kind of join?
  "    WHERE class_time.class_id = class_id;\n" + 
  // Hard code in a filter for what student status to filter for
  "END;",
  "Procedure create_instructor_schedule created if it didn't already exist.");
  
  // We need all class times for a certain class
  runSQL("CREATE PROCEDURE IF NOT EXISTS `select_class_times`(\n" + 
  "  IN class_id INT UNSIGNED \n" + 
  ")\n" + 
  "BEGIN\n" + 
  "	\n" + 
  "    SELECT class_time_id, class_id, day_of_week_id, `group`, start_time, end_time\n" + 
  "    FROM class_time\n" + 
  "    WHERE class_time.class_id = class_id;\n" + 
  "END;",
  "Procedure create_instructor_schedule created if it didn't already exist.");
