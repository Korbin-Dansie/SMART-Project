...
function createTables() {
  ...
  sql = con.execute(sql, function (err, results, fields) {
      if (err) {
        console.log(err.message);
        throw err;
      } else {
        console.log("database.js: table undefined created if it didn't exist");
      }
    });  
} // end of createTables()
  ...
  runSQL("CREATE PROCEDURE IF NOT EXISTS `create_user`(\n" +
        ...
        "BEGIN\n" +
          ...
          "IF nCount = 0 THEN\n" +
            ...
            "VALUES (accountType, @@IDENTITY, salt, hashed_password, email, 0);\n" +
          "ELSE\n" +
            "SET result = 1;\n" +
          "END IF;\n" +
        "END;");
